# Face Access Control – Copilot Playbook
...
- **Supabase Access**: Initialize clients via `src/lib/supabaseClient.ts`; expect `SUPABASE_PROJECT_URL` and `SUPABASE_SECRET_KEY` in `.env.local`; all Supabase interactions must be `async/await` and typed.
